 ...
 **/
quint32
ibus_keyval_to_unicode (uint keyval)
{
  int min = 0;
  int max = sizeof (ibus_keysym_to_unicode_tab) / sizeof (ibus_keysym_to_unicode_tab[0]) - 1;
  int mid;
  ...
  while (max >= min) {
    mid = (min + max) / 2;
    if (ibus_keysym_to_unicode_tab[mid].keysym < keyval)
      min = mid + 1;
    else if (ibus_keysym_to_unicode_tab[mid].keysym > keyval)
      max = mid - 1;
    else {
      /* found it */
      return ibus_keysym_to_unicode_tab[mid].ucs;
    }
  }
 ...
 **/
uint
ibus_unicode_to_keyval (quint32 wc)
{
  int min = 0;
  int max = sizeof (ibus_unicode_to_keysym_tab) / sizeof (ibus_unicode_to_keysym_tab[0]) - 1;
  int mid;
  ...
  while (max >= min) {
    mid = (min + max) / 2;
    if (ibus_unicode_to_keysym_tab[mid].ucs < wc)
      min = mid + 1;
    else if (ibus_unicode_to_keysym_tab[mid].ucs > wc)
      max = mid - 1;
    else {
      /* found it */
      return ibus_unicode_to_keysym_tab[mid].keysym;
    }
  }
